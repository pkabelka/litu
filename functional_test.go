...
func TestReduceRightInt(t *testing.T) {
	x := []int{1, 2, 3}

	res := ReduceRight(x, func(acc int, e int) int {
		return acc + e
	}, 0)
	if res != 6 {
		t.Errorf("res = %v, want %v", res, 6)
...
func TestReduceRightFloat(t *testing.T) {
	x := []float64{1, 2, 3}

	res := ReduceRight(x, func(acc float64, e float64) float64 {
		return acc + e
	}, 0)
	if res != 6 {
		t.Errorf("res = %v, want %v", res, 6)
